    ...
    public class RunArchiveIngestionCommandHandler : IRequestHandler<RunArchiveIngestionCommand, ArchiveIngestionRunResult>
    {
        ...
        public async Task<ArchiveIngestionRunResult> Handle(RunArchiveIngestionCommand command, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (batch is null)
                {
                    ...
                    {
                        Id = Guid.NewGuid(),
                        MetadataFilePath = options.MetadataRemotePath,
                        MetadataChecksum = checksum,
                        StartedAt = DateTimeOffset.UtcNow,
                        RecordsDiscovered = records.Count,
                        Status = IngestionBatchStatus.Running,
                        CreatedDate = DateTime.UtcNow,
                        CreatedBy = $"ingestion:{command.TriggeredBy}",
                        IsActive = true
                    }; db.ArchiveIngestionBatches.Add(batch);
                }
        ...
        private async Task ProcessRecordAsync(ArchiveIngestionBatch batch, ArchiveImportRecord record, CancellationToken cancellationToken)
        {
            ...
            if (employee is null)
            {
                ...
                {
                    Id = Guid.NewGuid(),
                    KellyId = record.KellyId,
                    EmployeeName = record.EmployeeName,
                    MaskedTaxId = record.MaskedTaxId,
                    StorageStatus = ArchiveStorageStatus.Processing,
                    CreatedDate = DateTime.UtcNow,
                    CreatedBy = nameof(RunArchiveIngestionCommand),
                    IsActive = true
                }; db.EmployeeArchives.Add(employee);
            }
            var blobName = BuildBlobName(record); document ??= new ArchiveDocument
            {
                Id = Guid.NewGuid(),
                ...
                DocumentPeriod = record.DocumentPeriod,
                OriginalFileName = Path.GetFileName(record.RemoteFilePath),
                FileSizeBytes = record.FileSizeBytes,
                ContentType = ContentTypeFor(record.RemoteFilePath),
                BlobContainer = options.BlobContainer,
                BlobName = blobName,
                SourcePath = record.RemoteFilePath,
            ...
            try
            {
                ...
                db.ArchiveAuditEvents.Add(new ArchiveAuditEvent
                {
                    Id = Guid.NewGuid(),
                    OccurredAt = DateTimeOffset.UtcNow,
                    ActorId = "system:archive-ingestion",
                    ActorDisplayName = "PARS ingestion command",
                    Action = ArchiveAuditAction.Ingested,
                    Outcome = "Success",
                    EmployeeArchive = employee,
                    ArchiveDocument = document,
                    Details = $"Transferred {record.RemoteFilePath} to {options.BlobContainer}/{blobName}.",
            ...
            {
                document.Status = ArchiveDocumentStatus.Failed; document.FailureReason = exception.Message; employee.StorageStatus = ArchiveStorageStatus.Review; employee.StatusDetail = $"Transfer failed for {record.RemoteFilePath}.";
